...
| 2026-09-25 | Menu lateral: a aba do item ativo agora é um elemento só que desliza até o item novo e ajusta a altura (300ms, `useAbaDeslizante` em `Sidebar.tsx`, mede o link com `aria-current` e remede com ResizeObserver); a cor do texto troca junto. Nasce no lugar ao abrir o app (sem deslizar do topo). "Reduzir animações" (app ou sistema) deixa a troca instantânea pela regra global do CSS. Gaveta mobile mantém a pílula estática. |
| 2026-09-25 | Barras de rolagem do app estilizadas (`src/index.css`): finas, em pílula, sem setas, na cor `--border-strong` do tema (mais fraca em repouso, forte no hover); Firefox via `scrollbar-width`/`scrollbar-color`. Vale para o quadro de seguimento, tabelas e barra lateral, nos três temas. |
| 2026-09-25 | Correções: rótulos de campo fora de `GrupoCampos` voltaram a alinhar à esquerda (sobrara um `self-end` da mudança do subgrid; aparecia nos filtros do Seguimento). Página inteira rolava na horizontal em telas médias: textos `sr-only` (posição absoluta) dentro de colunas fora da tela escapavam do contêiner com rolagem; todos os contêineres `overflow-x-auto` agora são `relative`. |
